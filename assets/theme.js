...
var createFocusTrap = function createFocusTrap(elements, userOptions) {
  ...
  var tryFocus = function tryFocus(node) {
    ...
    state.mostRecentlyFocusedNode = node;
     ...
     if(node != document.querySelector('.quick-search__input.frenzy_search_suggest')){
  ...
  var checkFocusIn = function checkFocusIn(e) {
   //  if(document.querySelector('input#search.quick-search__input.frenzy_search_suggest') != e.target){
...
if(window.FormData && formElement){
  ...
  return fetch(paths.add, {
    ...
    // },
    body: formdata ? formdata : serialize(form)
  }).then(r => r.json()).then(res => {
    if (res.status == "422") {
      const error = {
        code: 422,
...
function ProductForm(container, form, prod) {
    ...
    const onSubmit = event => {
      event.preventDefault(); 
      event.dataset = getProductFormEventData();
      setIdInputValue(event.dataset.variant.id); // Set the correct variant ID  
    ...
    const addToCart = (event) => {
      ...
      // Append the correct product variant and quantity to the data
      data.append('id', event.dataset.variant.id);
      data.append('quantity', getQuantity());
  
      // Append any custom properties if they exist
      const properties = getProperties();
      if (properties) {
        for (const key in properties) {
          data.append(`properties[${key}]`, properties[key]);
        }
      }
  
      // Use fetch API to add the product to the cart without page reload
      fetch('/cart/add.js', {
        method: 'POST',
        body: data
      })
  ...
  onLoad() {
    ...
    cart_cust =  this;
      window.frenzyCartCallBack = function(){
        cart_popup_status = 'frenzy';
          r$1("quick-cart:open");
           cart_cust.scrollUpQuickCart();
           cart_cust.refreshQuickCart();
           // cart_cust.updateCartCount(cart.item_count);
          if(document.querySelectorAll('.frenzy_bundle_cta_atc').length > 0){
            document.querySelector('.frenzy_bundle_cta_atc').querySelector('.frenzy_loading_spinner') ? document.querySelector('.frenzy_bundle_cta_atc').querySelector('.frenzy_loading_spinner').remove() : '';
          }
        }
  ...
  refreshQuickCart() {
    ...
    makeRequest("GET", url).then(response => {
      ...
      this._initCrossSells();
        frenzyAsyncItems();
        setTimeout(function(){
          // if(document.querySelector(selectors$r.items).querySelectorAll('.quick-cart__item').length >= 1){
          //    document.querySelector('.quick-cart__form').style.maxHeight = '530px'; 
          //    document.querySelector('.quick-cart__items').style.paddingBottom = '5px';
          // }
          if(cart_popup_status == 'frenzy'){
            document.querySelector('.cart span[data-js-cart-count]').innerHTML = document.querySelector(selectors$r.cartCount).innerHTML;
          }
        },700);
    });
  },
